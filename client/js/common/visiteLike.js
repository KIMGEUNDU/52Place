...
import { attr, getNode, getNodes, insertLast, userDataReview } from '../../lib/index.js';
...
const upLoadImg = getNodes('.uploadImg');
...
const keyEscapeSrc = reviewData[0].image[0].src;
const keyEscapeAlt = reviewData[0].image[0].alt;
...
function handleNewReviewAdd(e) {
	e.preventDefault();
	try {
		if (reviewText.value.length >= 30) {
			userDataReview
				.post(URL, keyEscapeImg(keyEscapeSrc, keyEscapeAlt))
				.then(() => {
